...
Finally, to give evidence that the core methods behind openWakeWord (i.e., pre-trained speech embeddings and high-quality synthetic speech) are effective across a wider range of wake word/phrase structure and length, the table below shows the performance on the [Fluent Speech Commands](https://paperswithcode.com/sota/spoken-language-understanding-on-fluent) test set using an openWakeWord model and the baseline method shown in a [related paper by the dataset authors](https://arxiv.org/abs/1910.09463). Note that due to fundamentally different data preparation, training, and evaluation approaches between the two models the overall conclusion should only be that openWakeWord is likely equally capable compared to a more complex (and significantly larger) model in the task of spoken language understanding (SLU).
